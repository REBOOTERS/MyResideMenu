...
public class ResideMenu extends FrameLayout implements OnClickListener {
	...
	private AnimatorSet buildActivityUpAnimation(View target) {
		...
		if (target == scrollViewMenu) {
			scaleUp.play(ObjectAnimator.ofFloat(target, "translationX", -screenWidth));
		} else {
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (pressedState == PRESSED_DOWN) {
				...
				if (targetTranslateX > screenWidth * translateXParam) {
					targetTranslateX = (float) (screenWidth * translateXParam);
				}
				ViewHelper.setTranslationX(viewActivity, targetTranslateX);
				ViewHelper.setTranslationX(imageViewShadow, targetTranslateX);
				ViewHelper.setTranslationX(scrollViewMenu,
						(float) (targetTranslateX - (translateXParam) * screenWidth));
	...
	public void setTranslateXParam(float translateXParam) {
		...
		translateXParam = translateXParam > 1.0f ? 1.0f : translateXParam;
		translateXParam = translateXParam < 0.0f ? 0.0f : translateXParam;
	}
